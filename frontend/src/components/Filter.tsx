  ...
  return (
    <div class="row">
      <div class="col input-group input-group-sm wyl-filter">
        ...
        <select onChange={(event)=>{handleFilter("Known", event)}} class="form-select form-select-sm" title="Filter by Known" value={selectValue()}>
          <option value="" selected disabled>Known</option>
          <option value={1}>Known</option>
          <option value={0}>Unknown</option>
        </select>
        <select onChange={(event)=>{handleFilter("Now", event)}} class="form-select form-select-sm" title="Filter by Online" value={selectValue()}>
          <option value="" selected disabled>Online</option>
          <option value={1}>On</option>
          <option value={0}>Off</option>
        </select>
        <button onClick={handleReset} class="btn btn-sm btn-outline-primary" title="Reset filter">Reset</button>
      </div>
    </div>
  )
}
